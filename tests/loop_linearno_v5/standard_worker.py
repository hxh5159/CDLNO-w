...
def run(task,action,directory,report):
    ...
    contract=resolve_config(task,options=dict(
        architecture='partial_share_feature_gate_v5',topology_preset='p2_c2_r2_s2',
        residual_mode='operator_1_expert_1_over_r',actual_M=4,expert_count=2,expert_width=7),
        profile_overrides={'model.hidden':8,'model.heads':2,'model.ffn_ratio':1,
                           'training.epochs':2,'training.batch_size':1,'runtime.seed':17})
    directory=Path(directory)/run_directory_id(contract)
    ...
    if action in ('train','interrupt'):
        tokens += ['--linearno-loop','1',
                   '--linearno-loop-architecture','partial_share_feature_gate_v5',
                   '--linearno-loop-topology','p2_c2_r2_s2',
                   '--linearno-loop-residual-mode','operator_1_expert_1_over_r',
                   '--linearno-loop-dense-expert-count','2',
                   '--linearno-loop-dense-expert-width','7',
                   '--n-hidden','8','--n-heads','2','--linearno-rank','4',
                   '--mlp_ratio','1','--seed','17','--epochs','2','--batch-size','1']
    ...
    payload=dict(task=task,architecture='partial_share_feature_gate_v5',action=action,
        device=device,scope='SYNTHETIC original task loops',counts=counts,
        shape_facts=args._linearno_loop_config['loop_spec'],weights_hash=state_hash(run.model.state_dict()),
...
if __name__=='__main__':
    parser=argparse.ArgumentParser();parser.add_argument('task');parser.add_argument('action')
    parser.add_argument('directory',type=Path);parser.add_argument('report',type=Path)
    a=parser.parse_args();run(a.task,a.action,a.directory,a.report)
