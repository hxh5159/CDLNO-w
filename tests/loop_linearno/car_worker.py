...
from cdlno.linearno_loop.car_entry import CarRun,run_cli
...
def run(action,directory,signature):
    ...
    if action in ('train','interrupt'):
        tokens+=['--nb_epochs','3','--linearno-hidden','8',
            '--linearno-heads','2','--linearno-rank','4','--seed','19','--cfd_mesh','--fold_id','3']
    ...
    batches=[];runs=[];predictions=[];handles=[];checks=[]
    ...
    def complete(self,*a):
        ...
        return result
    from cdlno.linearno_loop.car_entry import evaluate
    ...
    try:
        with patch('cdlno.linearno_loop.car_entry.inspect_data',return_value=spec),patch('cdlno.linearno_loop.car_entry.load_data',return_value=(train_ds,test_ds,coef)),patch.object(CarRun,'prepare',prepare),patch.object(CarRun,'complete_epoch',complete),patch('cdlno.linearno_loop.car_entry.evaluate',eval_without_raw):
            run_cli(args)
    except Interrupted:
    ...
    metadata,manifest=inspect_checkpoint(directory,'latest' if action=='interrupt' else 'final')
    saved,state=read_pair(manifest)
    if action=='eval':
    ...
    return dict(loop_forward_checks=len(checks),preset=PRESET,mode=MODE,epoch=metadata['resume_state']['epoch'],global_step=metadata['resume_state']['global_step'],
