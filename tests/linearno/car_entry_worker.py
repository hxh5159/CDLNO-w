...
from cdlno.linearno.car_entry import CarRun,run_cli,constructor_kwargs
...
def run(action,directory):
    ...
    if action in ('train','interrupt'):
        tokens+=['--nb_epochs','3','--linearno-hidden','8',
            '--linearno-heads','2','--linearno-layers','2','--linearno-rank','4','--seed','19','--cfd_mesh']
    ...
    def complete(self,*a):
        ...
        return result
    from cdlno.linearno.car_entry import evaluate
    ...
    try:
        with patch('cdlno.linearno.car_entry.inspect_data',return_value=spec),patch('cdlno.linearno.car_entry.load_data',return_value=(train_ds,test_ds,coef)),patch.object(CarRun,'prepare',prepare),patch.object(CarRun,'complete_epoch',complete),patch('cdlno.linearno.car_entry.evaluate',eval_without_raw):
            run_cli(args)
    except Interrupted:
    ...
    if action!='eval':
        model=ShapeNetLinearNO(**constructor_kwargs(args._linearno_config));model.load_state_dict(state,strict=True);model.eval()
...
if __name__=='__main__':
    ...
    Path(report).write_text(json.dumps(run(action,Path(directory)),indent=2)+'\n')
