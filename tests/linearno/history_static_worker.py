"""R6 history integration: actual four exp main ASTs with synthetic tensors only.
...
def run(task, action, directory, report, signature):
    ...
    if action in ('train','interrupt'):
        argv+=['--epochs','3','--n-hidden','8','--n-heads','2','--n-layers','4',
               '--linearno-rank','4','--batch-size','2','--seed','17','--linearno-fair-run','1']
        argv += ['--linearno_latent_attnres',signature[1],'--linearno_history_k_conditioning',signature[3]]
    elif action=='resume':argv+=['--resume']
    elif action=='eval':argv+=['--eval','1']
    else:raise ValueError(action)
    args=parse_args(parser_for(task),task,argv)
    global StandardRun
    from cdlno.linearno.standard_entry import StandardRun as PureRun
    from cdlno.linearno_history.standard_entry import HistoryStandardRun, FairBaselineRun
    StandardRun=HistoryStandardRun if getattr(args,'linearno_family',None)=='linearno_history' else (FairBaselineRun if getattr(args,'_linearno_fair_run',False) else PureRun)
...
if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('task',choices=SHORT);p.add_argument('action');p.add_argument('directory',type=Path);p.add_argument('report',type=Path);p.add_argument('signature')
