...
from cdlno.linearno.air_entry import AirRun, run_cli, _restore_coef_norm, _constructor_kwargs
...
def run(action, directory):
    ...
    if action in ('train','interrupt','ensemble','ensemble_interrupt','ensemble_later_interrupt'):
        tokens += ['--epochs','3','--linearno-hidden','8','--linearno-heads','2',
            '--linearno-layers','2','--linearno-rank','4','--seed','901',
    ...
    if evaluation:
        ...
        from cdlno.linearno.air_entry import check_structure
        import copy
        ...
        try:
            ...
            try:inspect_checkpoint(directory/'member_000','final',AirfRANSLinearNO)
            except ValueError:negative_count += 1
    ...
    batches = []; predictions = []; states = []; handles = []
    ...
    try:
        with patch('cdlno.linearno.air_entry._data_root',return_value=Path('synthetic:in-memory')), \
             patch('cdlno.linearno.air_entry._data_spec',return_value=data_spec), \
             patch('cdlno.linearno.air_entry._load_dataset',side_effect=load), \
             ...
             patch('cdlno.linearno.air_entry._pressure_rL2_dataset',return_value={'SYNTHETIC':True}):
            run_cli(args)
    except Interrupted:
    ...
    for i in range(nmodel):
        ...
        metadata,path=inspect_checkpoint(member,'latest',AirfRANSLinearNO)
        _,state=read_pair(path,AirfRANSLinearNO); states.append(state)
        epochs.append(metadata['resume_state']['epoch']); resume_hash.append(digest(metadata['resume_state']))
        if not evaluation:
            model=AirfRANSLinearNO(**_constructor_kwargs(config));model.load_state_dict(state,strict=True);model.eval()
            ...
            whole=torch.load(member/'model',map_location='cpu',weights_only=False).eval()
    ...
    return dict(epoch=epochs[0],epochs=epochs,state_hash=hashes[0],member_hashes=hashes,loaded_hashes=hashes,
        ...
        metadata_immutable=not before or before==after,pointer=True,shape=list(predictions[0].shape),family='linearno',
        objective=config['values']['objective'],seed=args.seed,task=args.task,negative_count=negative_count)

if __name__=='__main__':
    action,directory,report=sys.argv[1:]
    Path(report).write_text(json.dumps(run(action,Path(directory)))+'\n')
