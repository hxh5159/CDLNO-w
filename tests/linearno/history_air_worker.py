...
from cdlno.linearno_history.air_entry import AirRun, run_cli, _restore_coef_norm, _constructor_kwargs
...
def run(action, directory, signature):
    ...
    if action in ('train','interrupt','ensemble','ensemble_interrupt','ensemble_later_interrupt'):
        tokens += ['--epochs','3','--linearno-hidden','8','--linearno-heads','2',
            '--linearno-layers','4','--linearno-rank','4','--seed','901',
    ...
    if evaluation:
        ...
        from cdlno.linearno_history.air_entry import check_structure
        import copy
    ...
    try:
        with patch('cdlno.linearno_history.air_entry._data_root',return_value=Path('synthetic:in-memory')), \
             patch('cdlno.linearno_history.air_entry._data_spec',return_value=data_spec), \
             patch('cdlno.linearno_history.air_entry._load_dataset',side_effect=load), \
             ...
             patch('cdlno.linearno_history.air_entry._pressure_rL2_dataset',return_value={'SYNTHETIC':True}):
            run_cli(args)
    except Interrupted:
...
if __name__=='__main__':
    ...
    Path(report).write_text(json.dumps(run(action,Path(directory),signature))+'\n')
