...
def provenance():
    ...
    from .ll7_projection import LL6_FILES, project
    paths={*(root/'cdlno/linearno_loop'/name for name in LL6_FILES),*(root/'linearno_loop').glob('*.py'),
           *(root/'cdlno/linearno').glob('*.py'),*(root/name for name in REPLACEMENTS)}
    paths.add(root/'cdlno/linearno_history/fair_run.py')
    sources={str(p.relative_to(root)):hashlib.sha256(project(str(p.relative_to(root)),p.read_text()).encode()).hexdigest() for p in sorted(paths)}
    normalized={}
    for p in sorted(paths):
        ...
        normalized[name]=dict(before=ast.dump(ast.parse(old)) if old else None,after=ast.dump(ast.parse(project(name,p.read_text()))))
