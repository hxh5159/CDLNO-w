"""Shared physical LinearNO blocks with SR 1/R and point-domain Block AttnRes."""
...
def require_implemented(mode):
    if type(mode) is not str or mode not in ('sr_1_over_r','rb_attnres'):
        raise ValueError('only sr_1_over_r and rb_attnres are implemented; LB is not implemented')
...
class LinearNOLoopCore(nn.Module):
    ...
    def validate_structure(self):
        ...
        if self.residual_mode=='rb_attnres':
            ...
            raise ValueError('SR must not register RB receiver parameters')
