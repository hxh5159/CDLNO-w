...
def main():
    tasks=os.environ.get('LL7_STANDARD_TASKS','airfoil darcy elasticity pipe ns plasticity').split()
    modes=os.environ.get('LL7_STANDARD_MODES',' '.join(RESIDUAL_MODES)).split()
    presets=os.environ.get('LL7_STANDARD_PRESETS',' '.join(PRESETS)).split()
    artifact=Path(tempfile.mkdtemp(prefix='loop-ll7-standard-',dir='/home/hwz/CDLNO-artifacts'))
    report_name=os.environ.get('LL7_STANDARD_MATRIX_REPORT','native-matrix.json')
    ...
    with ThreadPoolExecutor(max_workers=2) as pool:
        ...
        for future in as_completed(futures):
            ...
            (OUT/report_name).write_text(json.dumps(dict(artifact_root=str(artifact),completed=len(rows),
                expected=len(futures),rows=rows),indent=2)+'\n')
