...
artifact=Path(tempfile.mkdtemp(prefix='loop-ll7-industrial-',dir='/home/hwz/CDLNO-artifacts'))
...
def main():
 tasks=os.environ.get('LL7_TASKS','airfrans car').split();presets=os.environ.get('LL7_PRESETS',' '.join(PRESETS)).split();modes=os.environ.get('LL7_MODES',' '.join(RESIDUAL_MODES)).split()
 rows=[];name=os.environ.get('LL7_REPORT','industrial-matrix.json')
 with ThreadPoolExecutor(max_workers=2) as pool:
  futures=[pool.submit(job,t,p,m) for t in tasks for p in presets for m in modes]
  for f in as_completed(futures):
   row=f.result();rows.append(row);print(json.dumps({k:row[k] for k in ('task','preset','mode','seconds','exact')}),flush=True)
   (OUT/name).write_text(json.dumps(dict(artifact_root=str(artifact),expected=len(futures),completed=len(rows),rows=rows),indent=2)+'\n')
