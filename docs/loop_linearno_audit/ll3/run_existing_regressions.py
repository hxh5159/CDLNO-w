"""LL3 rerun of the unchanged LL0 test inventory: execute unchanged CPU tests in isolated processes."""
...
print('LL3_RESULT_JSON='+json.dumps(dict(tests=r.testsRun,failures=[str(t) for t,_ in r.failures],errors=[str(t) for t,_ in r.errors],skipped=[(str(t),why) for t,why in r.skipped])))
...
def run(job):
 ...
 log=(OUT/(name+'.log')).read_text(); match=re.search(r'LL3_RESULT_JSON=(.*)',log)
