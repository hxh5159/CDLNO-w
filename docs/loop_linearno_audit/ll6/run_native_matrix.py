...
def main():
    tasks=os.environ.get('LL6_TASKS','airfoil darcy elasticity pipe ns plasticity').split()
    modes=os.environ.get('LL6_MODES',' '.join(RESIDUAL_MODES)).split()
    presets=os.environ.get('LL6_PRESETS',' '.join(PRESETS)).split()
    artifact=Path(tempfile.mkdtemp(prefix='loop-ll6-matrix-',dir='/home/hwz/CDLNO-artifacts'))
    report_name=os.environ.get('LL6_MATRIX_REPORT','native-matrix.json')
