...
from .core import require_sr, topology


def validate_arguments(kind,kw):
    require_sr(kw['residual_mode'])
...
def build_from_config(config):
    ...
    Intended for LL3 synthetic verification only. No parser, filesystem output,
    task data, weight loading or legacy dispatch. Unsupported modes fail first.
    """
    checked=validate_config(config)
    require_sr(checked['loop_spec']['residual_mode'])
...
def common_initial_state(config):
    ...
    This is NOT an RB/LB model or factory: it returns a detached state_dict and
    never an executable unsupported model. Future routers must be added after
    this common initialization using their independent recorded seed.
    """
