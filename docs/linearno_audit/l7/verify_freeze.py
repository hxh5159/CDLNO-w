"""L7 read-only source/classification preservation check against L7 and L0."""
import ast
import hashlib
import json
...
baseline = load('docs/linearno_audit/l7/baseline.json')
current = {}
...
new_allowed = {'cdlno/linearno/shapenet.py', 'Car-Design-ShapeNetCar/models/LinearNO.py',
               'cdlno/linearno/car_metrics.py', 'tests/linearno/test_shapenet_model.py',
               'tests/linearno/test_car_metrics.py', 'docs/LINEARNO_L7_REPORT.md'}
for path in new:
    assert path in new_allowed or path.startswith('docs/linearno_audit/l7/'), ('unexpected new file', path)
...
assert git('diff') == (ROOT/'docs/linearno_audit/l7/before.diff').read_text()
old_industrial = [r['path'] for r in baseline['files'] if r['path'].startswith(('Airfoil-Design-AirfRANS/', 'Car-Design-ShapeNetCar/'))
...
print(json.dumps(dict(check_status='PASS', stage_status='BLOCKED pending C08',
    ...
    changed=changed, new_files=new, new_ignored=[], tracked_diff_equal_L7_start=True,
    old_industrial_source_byte_identical=old_industrial, l0_files=len(l0), l0_changes=l0_changes,
