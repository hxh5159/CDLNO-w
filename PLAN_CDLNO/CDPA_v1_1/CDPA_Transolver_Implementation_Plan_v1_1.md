# CDPA 神经算子：基于 Transolver 的完整修改计划 v1.1

日期：2026-09-13。用途：交给 Codex 在 Transolver 仓库执行模型修改。

v1.1 补充用户指定的 CUDA 12.8 / PyTorch 2.11 环境、八任务依赖核查、来源批处理与逐层 CDPA 的完整成本、LRSA 实测对照及全对话约束复核。v1 的模型数学、默认 2+6 和 entry 主实验不变。随附 requirements、constraints 和独立环境预检脚本；这些不是新模型实现。
...
\left(QK_s^\top/\sqrt{d_h}\right)V_s\right]W_O.
\]
