...
while (($#)); do
    case "$1" in
        --seed)
            (($# >= 2)) || { printf 'Missing value for --seed\n' >&2; exit 2; }
            seed="$2"
            shift 2
            ;;
        --seed=*)
            ...
            ;;
        --gpu)
            (($# >= 2)) || { printf 'Missing value for --gpu\n' >&2; exit 2; }
            gpu="$2"
            shift 2
            ;;
        --gpu=*)
        ...
        *)
            ...
            printf 'Only --seed and --gpu may be overridden.\n' >&2
            exit 2
            ;;
    esac
done
...
export CDLNO_REPO_ROOT="$repo_root"
export CDLNO_RUNS_ROOT="${LR12_SR_RUNS_ROOT:-$repo_root/output/linearno_loop_v3/lr_12_sr}"

entry="$repo_root/tran_evaluate/linearno_loop_v3/matched_v1/$task.sh"
[[ -f "$entry" ]] || { printf 'Missing V3 task launcher: %s\n' "$entry" >&2; exit 2; }

exec bash "$entry" "$action" \
    --linearno-loop-architecture operator_latent_adapter_v3 \
    --linearno-loop-cost-profile matched_v1 \
